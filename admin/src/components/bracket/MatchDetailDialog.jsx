...
export default function MatchDetailDialog({ match, open, onClose, onSubmitResult, onSchedule, submitting }) {
  ...
  const [scheduleData, setScheduleData] = useState({ startTime: "", venue: "" });
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4 py-2">
          ...
          {/* Schedule form (for unscheduled matches without a start time) */}
          {match.status === "unscheduled" && !match.startTime && !isBye && teamA && teamB && (
            <div className="space-y-3 border-t border-border pt-3">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Schedule Match</p>
              ...
              <Button onClick={handleSchedule} disabled={submitting} size="sm" className="w-full bg-gradient-to-r from-indigo-500 to-purple-600 text-white">
                {submitting && <Loader2 className="w-3 h-3 mr-1.5 animate-spin" />} Schedule
              </Button>
            </div>
          )}

          {/* Result submission (for scheduled/live matches) */}
          {!isBye && !isCompleted && teamA && teamB && (match.status === "upcoming" || match.status === "live") && (
            <div className="space-y-3 border-t border-border pt-3">
