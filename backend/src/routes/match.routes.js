...
const { attachTenant, injectClubId } = require('../middlewares/tenant.middleware');
...
// Basic match CRUD
router.post('/', authenticate, clubManagerOnly, attachTenant, injectClubId, validate(matchValidators.createMatchSchema), matchController.create);
router.get('/', authenticate, attachTenant, paginate, matchController.getAll);
router.get('/live/:clubId', matchController.getLive);
